...
namespace IFC.Examples
{
	internal class BeamTessellated : Beam
	{
		...
		{
			base.GenerateData(db, building, true);
		}
	}
	internal class BeamExtruded : Beam
	{
		protected override void GenerateData(DatabaseIfc db, IfcBuilding building)
		{
			base.GenerateData(db, building, false);
		}
	}
	internal abstract class Beam: IFCExampleBase
	{
		protected void GenerateData(DatabaseIfc db, IfcBuilding building, bool tessellated)
		{	 
			IfcBeam beam = GenerateIPE200(db, building, tessellated);
		}
		...
		private IfcBeam GenerateIPE200(DatabaseIfc db, IfcBuilding building, bool tessellated)
		{
			...
			IfcBoundedCurve boundedCurve = IfcBoundedCurve.ConvertCurve(db, pc);
			...
			return new IfcBeam(building,null,new IfcProductDefinitionShape(new IfcShapeRepresentation (extrudedAreaSolid))) { GlobalId = "0EF5_zZRv0pQPddeofU3KT", Name = "ExampleBeamName",Description = "ExampleBeamDescription", Tag = "Tag" }; 
		}
	}
}
